...
export function buildProcessorPrompt(
  ...
  return [
    ...
    `- 文件隔离总则：可读文件仅限本轮 pendingInbox、对应日期的目标日记、${processor}/research-tasks.json 和 ${processor}/last-run.json；可写文件仅限 ${staging}/、${processor}/、${diary}/ 和 ${ideas}/ 的本轮目标文件。除此之外不得读取、列出、搜索、测试、创建、修改或删除任何文件。`,
    `- 内容整理阶段不得读取或写入 ${research}/；研究简报只能由独立 research runner 处理。`,
