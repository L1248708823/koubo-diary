...
文件隔离总则：可读文件仅限本轮 `pendingInbox`、对应日期的目标日记、`PROCESSOR_DIR/research-tasks.json`、`PROCESSOR_DIR/last-run.json` 以及已知完整路径的 `IDEAS_DIR/`、`RESEARCH_DIR/` 目标文件；可写文件仅限 `STAGING_DIR/`、`PROCESSOR_DIR/`、日记目录、顶层 `IDEAS_DIR/` 和 `RESEARCH_DIR/` 的本轮目标文件。除此之外不得读取、列出、搜索、测试、创建、修改或删除任何文件。
