...
export function buildProcessorPrompt(
  ...
  return [
    ...
    `  想法候选：\n${ideaCandidates}`,
    `  研究候选：\n${researchCandidates}`,
    "- 研究任务记录必须包含 task_id、source_diary 或 source_idea、question、status、created_at、updated_at；同一问题更新原任务，问题目标或范围变化时创建新任务并填写 related_task_ids 或 related_briefs；时间使用 round_id 中的时间。",
    ...
    "4. 写回以日记为轴；可选一个或多个想法并互链（日记=钩子+链接，想法=全文）；待查只登记确有外部证据需求的研究任务，不在本阶段联网。",
    "日记正文保留用户原话的口语、情绪、疑问和语气，不是转述也不是总结。内容层一律不动，只允许处理排版层。",
    ...
    "5. 轻整理分层：排版层积极，明显错别字、标点错误、断句和段落都修；内容层极度保守，情绪、语气、口语、疑问句、犹豫、自我怀疑、未决问题、强调重复和语气符号（？？？~~~）一律原样保留，不得把口语改书面、不得把疑问改陈述、不得升格文风、不得删情绪、不得改写总结、不得补充未说内容。确定错了才改，拿不准就保留。",
